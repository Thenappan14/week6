describe('Resource Management Frontend', () => {
  ...
  it('should add a new resource', ()=>{
    ...
    // Fill in the form
    cy.get('#name').type('Test Resource');
    cy.get('#location').type('Test Location');
    cy.get('#description').type('Test Description');
    cy.get('#owner').type('test@example.com', {force: true});

    // Click the add button
    cy.get('button.btn-primary').filter(':contains("Add New Resource")').click();
  ...
  it('should update an existing resource', ()=>{
    ...
    cy.get('#tableContent').contains('Test Location').should('not.exist');
  });
  
  it('should delete a resource', ()=>{
    cy.visit(baseUrl);
  
    // Click the delete button for the resource
    cy.get('button.btn-danger').filter(':contains("Delete")').last().click();
  
    // Verify the resource is deleted from the table
